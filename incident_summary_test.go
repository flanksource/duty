...
var _ = ginkgo.Describe("Check incident_summary view", ginkgo.Ordered, func() {
	ginkgo.It("Should query incident_summary view", func() {
		...
		Expect(responders[0].ID).To(Equal(dummy.JohnDoe.ID.String()))
		Expect(responders[0].Name).To(Equal(dummy.JohnDoe.Name))
		Expect(responders[1].ID).To(Equal(dummy.JohnWick.ID.String()))
		Expect(responders[1].Name).To(Equal(dummy.JohnWick.Name))

		var commenters []actor
		err = json.Unmarshal(commentersRaw, &commenters)
		Expect(err).ToNot(HaveOccurred())
		Expect(len(commenters)).To(Equal(2))
		Expect(commenters[0].ID).To(Equal(dummy.JohnDoe.ID.String()))
		Expect(commenters[0].Name).To(Equal(dummy.JohnDoe.Name))
		Expect(commenters[1].ID).To(Equal(dummy.JohnWick.ID.String()))
		Expect(commenters[1].Name).To(Equal(dummy.JohnWick.Name))
	})
})
