...
var _ = ginkgo.Describe("Dummy fixtures with agent annotation", ginkgo.Ordered, func() {
	ginkgo.It("should load echo-server configs with agent set", func() {
		...
		// correctly parses the "dummy.flanksource.com/agent" annotation from Kubernetes resources
		// and sets the AgentID field on the resulting ConfigItem.
		//
		// The echo-server.yaml fixture contains 3 resources (Deployment, ReplicaSet, Pod) that each
		// have the annotation "dummy.flanksource.com/agent: ac4b1dc5-b249-471d-89d7-ba0c5de4997b"
		// which corresponds to the HomelabAgent. We verify that all 3 configs are loaded into the
		// database with the correct AgentID set.
