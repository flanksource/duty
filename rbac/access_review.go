...
func RunSubjectAccessSearch(ctx context.Context, req SubjectAccessSearchRequest) (SubjectAccessSearchResponse, error) {
	if err := req.Validate(); err != nil {
		return SubjectAccessSearchResponse{}, err
	}
	...
	for _, resourceType := range req.ResourceTypes {
		...
		case "playbook":
			...
			if err := query.Order("COALESCE(title, name) ASC").Find(&playbooks).Error; err != nil {
				return SubjectAccessSearchResponse{}, ctx.Oops().Wrapf(err, "failed to list playbooks for subject access search")
			}
			...
			for _, playbook := range playbooks {
				r, err := RunSubjectAccessReview(ctx, SubjectAccessReviewRequest{
					...
					Action:   req.Action,
					Resource: SubjectAccessReviewResource{Playbook: playbook.ID.String()},
				})
				if err != nil {
					return SubjectAccessSearchResponse{}, err
				}
				...
				for _, x := range r {
					if x.Allowed {
						results = append(results, SubjectAccessSearchResult{ResourceType: "playbook", ID: playbook.ID.String()})
					}
				}
			}
		...
		case "view":
			...
			if err := query.Order("name ASC").Find(&views).Error; err != nil {
				return SubjectAccessSearchResponse{}, ctx.Oops().Wrapf(err, "failed to list views for subject access search")
			}
			...
			for _, view := range views {
				r, err := RunSubjectAccessReview(ctx, SubjectAccessReviewRequest{
					Subjects: []string{req.Subject},
					Action:   req.Action,
					Resource: SubjectAccessReviewResource{View: view.ID.String()},
				})
				if err != nil {
					return SubjectAccessSearchResponse{}, err
				}
				...
				for _, x := range r {
					if x.Allowed {
						results = append(results, SubjectAccessSearchResult{ResourceType: "view", ID: view.ID.String()})
					}
				}
			}
		...
		case "connection":
			...
			if err := query.Order("name ASC").Find(&connections).Error; err != nil {
				return SubjectAccessSearchResponse{}, ctx.Oops().Wrapf(err, "failed to list connections for subject access search")
			}
			...
			for _, connection := range connections {
				r, err := RunSubjectAccessReview(ctx, SubjectAccessReviewRequest{
					Subjects: []string{req.Subject},
					Action:   req.Action,
					Resource: SubjectAccessReviewResource{Connection: connection.ID.String()},
				})
				if err != nil {
					return SubjectAccessSearchResponse{}, err
				}
				...
				for _, x := range r {
					if x.Allowed {
						results = append(results, SubjectAccessSearchResult{ResourceType: "connection", ID: connection.ID.String()})
					}
				}
			}
		}
	...
	return SubjectAccessSearchResponse{
		Subject:       req.Subject,
		Action:        req.Action,
...
func RunSubjectAccessReview(ctx context.Context, req SubjectAccessReviewRequest) ([]SubjectAccessReviewResult, error) {
	...
	for _, subject := range subjects {
		...
		var allowed bool
		if req.Resource.Global != "" {
			allowed = Check(ctx, subject, req.Resource.Global, req.Action)
		} else {
			allowed = HasPermission(ctx, subject, resourceAttr, req.Action)
		}
...
func resolveAccessReviewSubjects(ctx context.Context, subjects []string) ([]string, error) {
	...
	if err := ctx.DB().
		...
		Where("id != ''").
		Order("id").
		Pluck("id", &allSubjects).Error; err != nil {
