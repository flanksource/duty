...
CREATE MATERIALIZED VIEW IF NOT EXISTS check_status_summary AS
    SELECT
        ...
        COUNT(*) FILTER ( WHERE status = TRUE) as passed,
        COUNT(*) FILTER ( WHERE status = FALSE) as failed,
        MAX(time) as last_check,
        MAX(time) FILTER (WHERE status = TRUE) as last_pass,
        MAX(time) FILTER (WHERE status = FALSE) as last_fail
    FROM
        check_statuses
    WHERE
