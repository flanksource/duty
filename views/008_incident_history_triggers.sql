...
CREATE
OR REPLACE FUNCTION insert_incident_created_in_incident_history () RETURNS TRIGGER AS $$
BEGIN
...
CREATE
OR REPLACE TRIGGER incident_to_incident_history
...
CREATE
OR REPLACE FUNCTION insert_responder_created_in_incident_history () RETURNS TRIGGER AS $$
BEGIN
...
CREATE
OR REPLACE TRIGGER responder_to_incident_history
...
CREATE
OR REPLACE FUNCTION insert_evidence_created_in_incident_history () RETURNS TRIGGER AS $$
DECLARE
incident_id UUID;
BEGIN
...
CREATE
OR REPLACE TRIGGER evidence_to_incident_history
...
CREATE
OR REPLACE TRIGGER responder_comment_to_incident_history
AFTER INSERT ON comments FOR EACH ROW WHEN (NEW.responder_id IS NOT NULL)
EXECUTE PROCEDURE insert_responder_comment_in_incident_history ();

-- Insert hypothesis creation updates in incident_histories
CREATE
OR REPLACE FUNCTION insert_hypothesis_created_in_incident_history () RETURNS TRIGGER AS $$
BEGIN
...
CREATE
OR REPLACE TRIGGER hypothesis_status_to_incident_history
AFTER INSERT ON hypotheses FOR EACH ROW
EXECUTE PROCEDURE insert_hypothesis_created_in_incident_history ();

-- Insert hypothesis status updates in incident_histories
CREATE
OR REPLACE FUNCTION insert_hypothesis_status_update_in_incident_history () RETURNS TRIGGER AS $$
BEGIN
...
OR REPLACE TRIGGER hypothesis_status_to_incident_history
AFTER
UPDATE ON hypotheses FOR EACH ROW WHEN (OLD.status IS DISTINCT FROM NEW.status)
